...
- `anti-pet/`：反 AI 宠物“十神”，主动向小智提问，测试它的记忆、逻辑、事实性和稳定性。
...
| `anti-pet` | 十神 | ESP32-S3-LCD-EV-BOARD-2 | `/dev/ttyUSB0` | 屏幕“挑战小智”按钮；首次自动唤醒小智 |

“你好，十神”需要单独训练并集成 WakeNet 模型。在模型可用前，十神使用屏幕按钮
主动发起会话。
...
默认未启用“你好，十神”WakeNet，因为仓库中尚未包含该自定义模型。
...
1. 确认屏幕完整显示“十神”界面并连接 Wi-Fi。
2. 点击“挑战小智”。
3. 首次会话确认十神先说“你好，小智”，然后显示正在等待小智回答。
4. 确认十神的 ASR 识别到小智回复“我在”；只有确认成功后才生成并播报挑战问题。
5. 如果未听到“我在”，确认界面提示重试，并且十神没有直接开始挑战。
...
4. 点击十神的“挑战小智”：首次会话由十神说“你好，小智”，等待小智回复“我在”，
   握手成功后，十神继续监听，确认小智连续静音后才发出第一条挑战。
5. 等小智回答完毕后，再启动十神的下一轮挑战；同一次运行中无需重复首次握手。
6. 连续测试至少 5 轮，检查上下文、误唤醒、ASR/TTS 失败和设备重启。
...
按钮启动十神。详见 [唤醒词更换与模型集成](docs/wake_word_change_plan.md)。
