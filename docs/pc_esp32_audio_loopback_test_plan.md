...
| `anti-pet/` | 大神 | LCD-EV | `/dev/ttyUSB0` | 屏幕“挑战小智”按钮 |
...
大神按钮会先让 LLM 生成一个短促的压力测试问题，再通过自身 TTS 扬声器主动
说给小智听。现阶段大神不依赖尚未交付的“你好，大神”WakeNet 模型。
...
1. 点击大神的“挑战小智”，确认它生成问题并完成 TTS。
2. 小智检测“你好，小智”后进入 ASR；若大神生成的问题未带唤醒词，则测试控制器
   应先播放唤醒短语，或用小智屏幕按钮进入聆听后再启动大神。
3. 确认小智完成 ASR → LLM → TTS，大神可通过屏幕按钮发起下一轮质疑。
